...
- **키보드 탐색** — `Ctrl+K`로 검색창, `↑↓`로 고르고 `Enter`로 열기, `Esc`로 터미널 복귀. 검색어 앞 기호로 상태 필터: `!` 작업 중 · `@` 입력 필요/읽지 않은 완료 · `#` 열린 탭 · `&` 실패
- **복사본 열기** — 우클릭 → 복사본으로 열기(`--fork-session`). 원본은 그대로 두고 갈라져 나온다
- **상태 표시** — 답변/작업 중(🟠 점멸) · 입력 필요(🟡 점멸, 권한 확인·질문) · 대기(🟢) · 종료(⚪)를 탭과 사이드바에 표시. 입력 필요로 바뀌면 완료와 같은 방식으로 알림
- **에이전트 / 래퍼 프로필** — 새 세션을 시작할 때 어떤 에이전트로 띄울지 고른다(Claude·Codex·Gemini 기본 제공). 실행 명령은 `래퍼 접두사 + 에이전트 명령`으로 합성되므로 커스텀 프록시도 등록할 수 있다
- **전역 환경변수** — 설정에서 `KEY=VAL;KEY2=VAL2` 형식으로 지정하면 모든 세션 실행에 적용 (예: `PYTHONUTF8=1`)
- **클립보드** — `Ctrl+V`/`Shift+Insert` 붙여넣기, 선택 후 `Ctrl+C` 복사, 우클릭은 선택 시 복사·미선택 시 붙여넣기 (한글 IME 상태에서도 동작)
- **토큰 상세** — 세션 우클릭 → 토큰 상세. 질문별 비용 순위로 어떤 질문이 토큰을 먹었는지 보고, 응답 하나가 막대 하나인 타임라인으로 캐시가 끊긴 턴(빨강)을 찾는다. 질문을 누르면 그 질문이 무슨 일을 했는지 흔적과 답변이 나온다
...
| `src-tauri/src/usage.rs` | 세션 파일 기반 사용량 통계, 요금제 한도(상태줄 → 사용량 API 순) |
