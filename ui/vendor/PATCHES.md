...
2026-09-14 재검토: Claude Code 2.1.270은 이제 앱 탭에서도 풀스크린 렌더러(`/tui fullscreen`)로 돌고 2026 괄호를
낸다. 윈도우에 내장된 ConPTY는 괄호를 통과시키지만 **순서를 지키지 않는다** — `?2026h`와 `?2026l`이 먼저 붙어서
나오고 프레임 본문은 약 14ms 뒤 괄호 밖으로 따로 온다(`pty.rs`의 `conpty_sync_output` 실측). 그래서 xterm이
동기화 출력으로 프레임을 한 번에 그리지 못한다. Windows Terminal의 ConPTY 1.24(`conpty.dll`+`OpenConsole.exe`)를
앱 옆에 두면 본문이 괄호 안에 같은 ms에 들어온다. 1.24는 시작할 때 터미널 질의(DA1)의 답을 기다리는데, xterm이
답하면 첫 출력이 내장과 같은 50ms 안팎이다(`conpty_first_output`, 답하지 않으면 3초).
